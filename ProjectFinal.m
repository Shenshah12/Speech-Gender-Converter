...
% Last Modified by GUIDE v2.5 17-Jan-2023 21:23:37
...
nbits=8;
nchannels=1;
speech=audiorecorder(Fs,nbits,nchannels);
...
stop(speech);
audiodata=getaudiodata(speech);
save myspeech.mat audiodata;
axes(handles.axes2)
plot(audiodata)
...
load myspeech.mat audiodata;
Fs=16000;
[f0,idx]=pitch(audiodata,Fs);
test = mean(abs(f0));
if (test>=165)&(test<=255 )
     voice="Female";
    set(handles.edit1,'string',voice);
    nsemitones = -6;
audioOut = shiftPitch(audiodata,nsemitones);
%audioOut=filter(BP,audiodata)
sound(audioOut,Fs)
axes(handles.axes2)
plot(audioOut)
end
